...
def _build_test_client(tmp_path: Path) -> TestClient:
    _reload_modules()
    os.environ["AEGIS_CONTROL_API_KEY"] = ""
...
def test_phase1_registration_and_inventory(tmp_path, monkeypatch):
    ...
    dc_id = dc.json()["node_id"]

    approve_dc = client.post(
        f"/api/control/dcs/{dc_id}/approve",
        params={"approved": True, "approved_by": "test_suite"},
    )
    assert approve_dc.status_code == 200
...
def test_phase4_template_dispatch_maps_agent_to_dc(tmp_path, monkeypatch):
    ...
    body = dispatch.json()
    assert body["resolved_dc_id"] == dc_id
    assert body["action"]["target_type"] == "dc"
    assert body["action"]["target_id"] == dc_id
